...
struct comPointsLarge
{
	bool  operator ()(CutPoint* a, CutPoint* b)const
	{
		if (a->getPosition().x() - b->getPosition().x() < -3*LIMIT)
		{
			return true;
		}
		else if (a->getPosition().x() - b->getPosition().x() < 3*LIMIT)
		{
			return a->getPosition().y() - b->getPosition().y() < -3*LIMIT;
		}
		return false;
	}
};
...
private:
	...
	void UpdateStructure(CutPoint* ptr_point_, std::vector<CutLine *>& str_line_, std::vector<CutLine *>& left_line_,
		std::vector<CutLine *>& righ_line_, std::vector<CutLine *>& cros_line_, std::vector<CutPoint*>&newpoints);
public:
	void storePathToPieces(std::vector<std::vector<std::pair<Vec3f, Vec3f>>>* pieces_list_,int id);
	void FindNewEvent(CutLine* down, CutLine* up, CutPoint* point, std::vector<CutPoint*> queue, std::vector<CutPoint*>& newpoints );
private:
};
