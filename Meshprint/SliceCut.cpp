...
void SliceCut::CutInPieces()//modified at 17/10/2017
{
	const std::vector<HE_face *>& faces = *(mesh_in_->get_faces_list());
...
void SliceCut::sweepThroughVertex()
{
	...
	while (!queue_event_.empty())
	{
		...
		do 
		{
			...
			for (int j = 0; j < cur_vert_->splitFace.size(); j++)
			{
				...
				hei_ = min(hei_, faces_[cur_vert_->splitFace[j]]->vertices_[1]->position().z());
			}
		...
		for (int k=0;k<layer_faces_.size();k++)
		{
			if (layer_faces_[k])
			{
				sweep_line_.insertSegment(cutFacet(faces_[k], hei_));
			}
		}
		pieces_list_->push_back(sweep_line_.polygonization());
	}
}
...
void SliceCut::cutFacetBymiddlePoint()
{
	const std::vector<HE_face *>& faces = *(mesh_in_->get_faces_list());
	face_middle_line_.resize(faces.size());
	for (int i = 0; i < faces.size(); i++)
	{
		
		if (abs(faces[i]->normal()*Vec3f(0.0,0.0,1.0))>0.98)
		{
			continue;
		}
		std::vector<Vec3f>  v_;
		...
		for (int j = 0; j < 3; j++)
		{
			v_.push_back(faces[i]->vertices_[j]);
		}
		...
		{
			face_middle_line_[i] = std::pair<Vec3f, Vec3f>(v_[1], pos_);
		}
		else
			face_middle_line_[i] = std::pair<Vec3f, Vec3f>( pos_,v_[1]);
	}
}

std::vector<std::vector < std::vector<std::pair<Vec3f, Vec3f>>>>* SliceCut::GetPieces()
{
